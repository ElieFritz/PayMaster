...
export class ReceiptMailerService {
  ...
  async sendInvoiceReceipt(invoice: Invoice, pdfBuffer?: Buffer): Promise<void> {
    ...
    const invoiceLink = frontendBaseUrl
      ? `${frontendBaseUrl}/p/${encodeURIComponent(invoice.id)}`
      ...
      lineItems.length > 0
        ? lineItems
            .map((line, index) => {
              ...
              return `
                <tr>
                  <td style="padding:10px 12px;border-bottom:1px solid #d6e3ff;background:${rowBackground};font-size:13px;color:#0f2e6d;font-weight:600;">${this.escapeHtml(line.name)}</td>
    ...
    return `
      ...
      <html lang="fr">
        ...
        <body style="margin:0;padding:0;background:#edf3ff;font-family:Arial,Helvetica,sans-serif;color:#0f2b61;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#edf3ff;padding:24px 12px;">
            <tr>
              <td align="center">
                <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="width:100%;max-width:640px;background:#ffffff;border-radius:20px;overflow:hidden;border:1px solid #cfe0ff;box-shadow:0 20px 45px rgba(10,42,107,0.18);">
                  <tr>
                    <td style="padding:28px 26px;background:linear-gradient(130deg,#0b3c9f 0%,#1b73ef 100%);">
                      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                        <tr>
                          <td style="font-size:28px;line-height:1;font-weight:800;color:#ffffff;letter-spacing:0.8px;">BOOST PERFORMERS</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          <td style="padding-top:12px;font-size:22px;line-height:1.3;font-weight:700;color:#ffffff;">Facture ${this.escapeHtml(invoice.reference)}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          <td style="padding:16px;background:#f4f8ff;border:1px solid #d6e4ff;border-radius:14px;">
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                              ...
                              <tr>
                                <td style="padding-top:6px;font-size:17px;font-weight:700;color:#12357f;">${this.escapeHtml(invoice.customerName)}</td>
                              ...
                              <tr>
                                <td style="padding-top:4px;font-size:13px;color:#4d6899;">${this.escapeHtml(invoice.customerEmail)}</td>
                    ...
                    <td style="padding:0 26px 10px 26px;">
                      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #d6e4ff;border-radius:14px;background:#f6f9ff;">
                        ...
                        <tr>
                          ...
                          <td style="padding:4px 16px 14px 16px;font-size:20px;color:#0e3280;font-weight:800;text-align:right;">${this.formatAmount(invoice.amount)} ${invoice.currency}</td>
                        </tr>
                      </table>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td style="padding:6px 26px 6px 26px;">
                      <p style="margin:0;font-size:13px;color:#294b84;line-height:1.6;">
                  ...
                  <tr>
                    <td style="padding:16px 26px 18px 26px;">
                      <a href="${this.escapeHtml(invoiceLink)}" style="display:inline-block;padding:12px 22px;border-radius:10px;background:#1154d4;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;">
                  ...
                  <tr>
                    <td style="padding:16px 26px 24px 26px;background:#f7faff;border-top:1px solid #dbe8ff;">
                      <p style="margin:0 0 8px 0;font-size:11px;line-height:1.6;color:#5f7296;">
                        ${this.escapeHtml(LEGAL_MENTIONS)}
                      </p>
                      <p style="margin:0;font-size:11px;line-height:1.6;color:#5f7296;">
